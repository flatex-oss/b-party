...
export const AnalyticsView: React.FC<AnalyticsViewProps> = ({
  ...
  const renderProgressBar = (
    ...
    return (
      <Card className="p-5 bg-white border border-slate-200/80 shadow-xs flex flex-col gap-4">
        ...
        {items.length === 0 ? (
          <p className="text-xs font-medium text-slate-400 italic py-2">{emptyText}</p>
        ) : (
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full">
        <Card className="p-4 text-center bg-white border border-slate-200/80 shadow-xs flex flex-col items-center justify-center gap-0.5">
          <span className="font-heading text-2xl sm:text-3xl font-extrabold text-orange-600 leading-none">
            {estimate.totalGuests}
          </span>
          <span className="text-xs font-semibold text-slate-500 flex items-center gap-1 mt-1">
            <Users size={13} /> Гостей
          </span>
        </Card>

        <Card className="p-4 text-center bg-white border border-slate-200/80 shadow-xs flex flex-col items-center justify-center gap-0.5">
          <span className="font-heading text-2xl sm:text-3xl font-extrabold text-indigo-600 leading-none">
          ...
          <span className="text-xs font-semibold text-slate-500 flex items-center gap-1 mt-1">
            <Wine size={13} /> Пьют бар
          </span>
        </Card>

        <Card className="p-4 text-center bg-white border border-slate-200/80 shadow-xs flex flex-col items-center justify-center gap-0.5">
          <span className="font-heading text-base sm:text-lg font-bold text-emerald-600 leading-none truncate max-w-full">
            {estimate.pizzaBreakdown[0]?.name.replace(/\s.*$/, '') || '—'}
          </span>
          <span className="text-xs font-semibold text-slate-500 flex items-center gap-1 mt-1">
            <Pizza size={13} /> Топ пицца
          </span>
        </Card>

        <Card className="p-4 text-center bg-white border border-slate-200/80 shadow-xs flex flex-col items-center justify-center gap-0.5">
          <span className="font-heading text-2xl sm:text-3xl font-extrabold text-amber-600 leading-none">
            {estimate.averageSpice} / 5
          </span>
          <span className="text-xs font-semibold text-slate-500 flex items-center gap-1 mt-1">
            <Flame size={13} /> Острота
          </span>
        </Card>
      </div>
      ...
      {estimate.allergiesSummary.length > 0 && (
        <div className="bg-rose-50/80 border border-rose-200 rounded-2xl p-5 shadow-xs flex flex-col gap-3">
          <h3 className="font-heading text-sm sm:text-base font-bold text-rose-900 flex items-center gap-2">
          ...
          <div className="flex flex-col gap-2">
            {estimate.allergiesSummary.map((item, idx) => (
              <div key={idx} className="bg-white border border-rose-200/70 rounded-xl p-3 text-xs sm:text-sm flex flex-wrap items-center gap-1.5">
      ...
      {estimate.avoidSummary.length > 0 && (
        <div className="bg-amber-50/80 border border-amber-200 rounded-2xl p-5 shadow-xs flex flex-col gap-3">
          <h3 className="font-heading text-sm sm:text-base font-bold text-amber-900 flex items-center gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            {estimate.avoidSummary.map((item, idx) => (
              <div
                key={idx}
                className="bg-white border border-amber-200 rounded-xl px-3 py-1.5 text-xs flex items-center gap-1.5"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
        {renderProgressBar('Пицца', estimate.pizzaBreakdown, <Pizza size={16} />)}
        {renderProgressBar('Суши и Роллы', estimate.sushiBreakdown, <span>🍣</span>)}
        {renderProgressBar('Алкоголь и Коктейли', estimate.alcoholBreakdown, <Wine size={16} />)}
        {renderProgressBar('Безалкогольные напитки', estimate.softBreakdown, <span>🧃</span>)}
        {renderProgressBar('Закуски и Снеки', estimate.snackBreakdown, <span>🍟</span>)}
      </div>

      {/* Guest Directory Accordion Card */}
      <Card className="p-5 sm:p-6 bg-white border border-slate-200/80 shadow-xs flex flex-col gap-4">
        ...
        {filteredResponses.length === 0 ? (
          <div className="text-center py-8 text-xs sm:text-sm font-medium text-slate-400 italic bg-slate-50 rounded-xl p-4">
          ...
          <div className="flex flex-col gap-2.5">
            {filteredResponses.map((guest) => {
              ...
              return (
                <div
                  key={guest.id}
                  className="bg-slate-50/70 border border-slate-200/80 rounded-xl overflow-hidden transition-all"
                >
                  <div
                    onClick={() => toggleGuestExpand(guest.id)}
                    className="p-3 sm:p-3.5 flex items-center justify-between cursor-pointer select-none hover:bg-slate-100/80 transition-colors gap-2"
                  >
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4 pt-3 border-t border-slate-200/60 bg-white text-xs flex flex-col gap-2.5">
                      <div>
                      ...
                      {guest.wishes && (
                        <div className="bg-slate-50 border border-slate-200/70 rounded-lg p-2.5 italic text-slate-700">
                          💬 "{guest.wishes}"
                        </div>
                      )}
                      ...
                      <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                        ...
                        <button
                          ...
                          className="text-[11px] font-medium text-rose-600 hover:text-rose-700 flex items-center gap-1 cursor-pointer py-1 px-2 rounded-lg hover:bg-rose-50"
                        >
      ...
      <div className="flex flex-wrap gap-2 justify-center pt-2">
        ...
        <Button
          ...
          className="text-xs flex items-center gap-1.5 bg-orange-600 hover:bg-orange-700"
        >
